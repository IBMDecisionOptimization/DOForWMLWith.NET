...
namespace SchedGoalProg
{
    public class SchedGoalProg
    {
        ...
        public double SolveGoalProgramming(log4net.ILog logger, int branchLimit)
        {

            // Objective 1
            logger.Info("");
            logger.Info(" ! ----------------------------------------------------------------------------");
            logger.Info(" ! STEP 1: Minimizing usage of expensive resources");
            logger.Info(" ! ----------------------------------------------------------------------------");
            ...
            for (int i = 0; i < nbJobs; i++)
            {
                for (int j = 0; j < nbMachines; j++)
                {
                    if (cp.IsPresent(machines[j][i].Name))
                    {
                        startSol.SetPresent(machines[j][i]);
                        startSol.SetStart(machines[j][i], cp.GetStart(machines[j][i].Name));
                    }
                }
            }
            ...
            logger.Info("");
            logger.Info(" ! ----------------------------------------------------------------------------");
            logger.Info(" ! STEP 2: Minimizing total cost");
            logger.Info(" ! ----------------------------------------------------------------------------");
            cp.Remove(obj1);
        ...
        public double SolveBasic(log4net.ILog logger, int branchLimit)
        {

            // Objective
            logger.Info("");
            logger.Info(" ! ----------------------------------------------------------------------------");
            logger.Info(" ! Minimizing total cost");
            logger.Info(" ! ----------------------------------------------------------------------------");
            IIntExpr costExpr = cp.IntExpr();
        ...
        public static void Main(String[] args)
        {
            ...
            String fileName = "../../../../../../Resources/cpo/goalprog_8_40_0.6_3.data";
            ...
            try
            {
                ...
                {
                    logger.Info("Solving in two steps using goal programming ...");
                    cost = gp.SolveGoalProgramming(logger, branchLimit);
                }
                else
                {
                    logger.Info("Solving in a single step  ...");
                    cost = gp.SolveBasic(logger, branchLimit);
                }
