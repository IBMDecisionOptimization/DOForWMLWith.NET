...
public class Talent
{
    ...
    private void Display(log4net.ILog logger)
    {
        logger.Info("Solution of idle cost "
                + (int)cp.ObjValue);
        logger.Info("Order:");
        for (int s = 0; s < numScenes; s++)
            logger.Info(" " + ((int)cp.GetValue(scene[s]) + 1));
        logger.Info("");
    ...
    public static void Main(String[] args)
    {
        ...
        String inputFile = "../../../../../../Resources/cpo/rehearsal.data";
        if (args.Length > 1)
            inputFile = args[1];
        Talent talent = new Talent(inputFile);
        cp.Solve();
        talent.Display(logger);
    }
}
