...
public sealed class QCPDual
{
    ...
    private static double[] getqconstrmultipliers(Cplex cplex, double[] xval, double tol, INumVar[] x, IRange[] qs)
    {
        ...
        IDictionary<INumVar, double> sol = new Dictionary<INumVar, double>();
        ...
        {
            IDictionary<INumVar, double> dslack = new Dictionary<INumVar, double>();
            for (ILinearNumExprEnumerator it = cplex.GetQCDSlack(qs[i]).GetLinearEnumerator(); it.MoveNext(); /* nothing */)
            {
                dslack[it.NumVar] = it.Value;
            }

            IDictionary<INumVar, double> grad = new Dictionary<INumVar, double>();
            bool conetop = true;
    ...
    public static void Main(string[] args)
    {
        ...
        try
        {
            ...
            x[1] = cplex.NumVar(double.NegativeInfinity, double.PositiveInfinity, "x2");
            x[2] = cplex.NumVar(0, 0.5, "x3");
            x[3] = cplex.NumVar(double.NegativeInfinity, double.PositiveInfinity, "x4");
            x[4] = cplex.NumVar(double.NegativeInfinity, double.PositiveInfinity, "x5");
            x[5] = cplex.NumVar(0, double.PositiveInfinity, "x6");
            x[6] = cplex.NumVar(double.NegativeInfinity, double.PositiveInfinity, "x7");
            ...
            IDictionary<INumVar, double> xmap = new Dictionary<INumVar, double>();
            for (int j = 0; j < x.Length; ++j)
            {
                xmap.Add(x[j], xval[j]);
            }
            ...
            {
                if (linear[i].LB <= double.NegativeInfinity)
                {
                    // <= constraint
                    if (rpi[i] > ZEROTOL)
                ...
                else if (linear[i].UB >= double.PositiveInfinity)
                {
                    // >= constraint
                    if (rpi[i] < -ZEROTOL)
                        throw new System.SystemException("Dual feasibility test failed for row " + linear[i]
            ...
            {
                if (quad[i].LB <= double.NegativeInfinity)
                {
                    // <= constraint
                    if (qpi[i] > ZEROTOL)
                ...
                else if (quad[i].UB >= double.PositiveInfinity)
                {
                    // >= constraint
                    if (qpi[i] < -ZEROTOL)
                        throw new System.SystemException("Dual feasibility test failed for quad " + quad[i]
            ...
            for (int j = 0; j < x.Length; ++j)
            {
                if (x[j].UB < double.PositiveInfinity)
                {
                ...
                if (x[j].LB > double.NegativeInfinity)
                {
            ...
            IDictionary<INumVar, double> kktsum = new Dictionary<INumVar, double>();
            for (int j = 0; j < x.Length; ++j)
            {
                kktsum.Add(x[j], 0.0);
            }
            ...
            // the dual values.
            logger.Info("Optimal solution satisfies KKT conditions.");
            logger.Info("   x[] = " + arrayToString(xval));
            logger.Info(" cpi[] = " + arrayToString(cpi));
            logger.Info(" rpi[] = " + arrayToString(rpi));
            logger.Info(" qpi[] = " + arrayToString(qpi));
        }
        catch (ILOG.Concert.Exception e)
        {
            logger.Info("IloException: " + e.Message);
