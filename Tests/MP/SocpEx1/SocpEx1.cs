...
public sealed class SocpEx1
{
    ...
    /// </remarks>
    private static IDictionary<IRange, double> Getsocpconstrmultipliers(Cplex cplex, ICollection<INumVar> vars, ICollection<IRange> rngs, IDictionary<INumVar, double> dslack)
    {
        // Compute full dual slack.
        IDictionary<INumVar, double> dense = new SortedDictionary<INumVar, double>(NumVarComparator);
        ...
        IDictionary<IRange, double> socppi = new SortedDictionary<IRange, double>(RangeComparator);
        foreach (IRange r in rngs)
        {
            INumExpr e = r.Expr;
            if ((e is IQuadNumExpr) && ((IQuadNumExpr)e).GetQuadEnumerator().MoveNext())
     ...
     * and false otherwise.
     */
    private static bool Checkkkt(log4net.ILog logger,
                                    Cplex cplex,
                                  IObjective obj,
                                  ...
                                  double tol)
    {
        IDictionary<INumVar, double> dslack = new SortedDictionary<INumVar, double>(NumVarComparator);
        IDictionary<INumVar, double> x = new SortedDictionary<INumVar, double>(NumVarComparator);
        IDictionary<IRange, double> pi = new SortedDictionary<IRange, double>(RangeComparator);
        IDictionary<IRange, double> slack = new SortedDictionary<IRange, double>(RangeComparator);
        ...
        logger.Info("x      = [");
        foreach (INumVar v in vars)
            logger.Info(string.Format(" {0}", x[v]));
        logger.Info(" ]");
        logger.Info("dslack = [");
        foreach (INumVar v in vars)
            logger.Info(string.Format(" {0}", dslack[v]));
        logger.Info(" ]");
        logger.Info("pi     = [");
        foreach (IRange r in rngs)
            logger.Info(string.Format(" {0}", pi[r]));
        logger.Info(" ]");
        logger.Info("slack  = [");
        foreach (IRange r in rngs)
            logger.Info(string.Format(" {0}", slack[r]));
        logger.Info(" ]");
        ...
        foreach (INumVar v in vars)
        {
            ...
            {
                logger.Info("Dual multiplier for " + v + " is not feasible: " + dslack[v]);
                return false;
            }
        }
        foreach (IRange r in rngs)
        {
            ...
            else if (r.LB > -double.MaxValue && pi[r] > tol)
            {
                logger.Info("Dual multiplier " + pi[r] + " for >= constraint");
                logger.Info(" " + r);
                logger.Info("not feasible.");
                return false;
            }
            else if (r.UB < double.MaxValue && pi[r] < -tol)
            {
                logger.Info("Dual multiplier " + pi[r] + " for <= constraint");
                logger.Info(" " + r);
                logger.Info("not feasible.");
        ...
        foreach (INumVar v in vars)
        {
            if (cone[v] == NOT_IN_CONE)
            {
                ...
                {
                    logger.Info("Invalid complementary slackness for " + v + ":");
                    logger.Info(" " + x[v] + " and " + dslack[v]);
                    return false;
                }
            }
        }
        foreach (IRange r in rngs)
        {
            ...
            {
                logger.Info("Invalid complementary slackness for");
                logger.Info(" " + r);
                logger.Info(" " + slack[r] + " and " + pi[r]);
                return false;
            }
        }
        ...
        IDictionary<INumVar, double> sum = new SortedDictionary<INumVar, double>(NumVarComparator);
        foreach (INumVar v in vars)
            sum[v] = 0.0;
        ...
        foreach (INumVar v in vars)
        {
            ...
            {
                logger.Info("Invalid stationarity " + sum[v] + " for " + v);
                return false;
            }
        }
    ...
    public static void Main(string[] args)
    {
        ...
        try
        {
            ...
            if (!Checkkkt(logger, cplex, obj, vars, rngs, cone, TESTTOL))
            {
                logger.Info("Testing of KKT conditions failed.");
            }
            else
            {
                logger.Info("KKT conditions are satisfied.");
                retval = 0;
            }
        }
